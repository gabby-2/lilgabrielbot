...
async function hideTagCommand(sock, chatId, senderId, messageText, replyMessage, message) {
    ...
    if (!isSenderAdmin) {
        await sock.sendMessage(chatId, { text: formatMessage('Only admins can use the .hidetag command.', '❌') }, { quoted: message });
        return;
    }
    ...
    if (replyMessage) {
        ...
        await sock.sendMessage(chatId, { text: formatMessage(messageText || 'Tagged members (excluding admins).', 'ℹ️'), mentions: nonAdmins });
    }
}
