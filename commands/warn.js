...
async function warnCommand(sock, chatId, senderId, mentionedJids, message) {
    try {
        ...
        try {
            ...
            if (!isBotAdmin) {
                await sock.sendMessage(chatId, { 
                    text: formatMessage('Please make the bot an admin first to use this command.', '⚠️')
                });
                return;
            }
            ...
            if (!isSenderAdmin) {
                await sock.sendMessage(chatId, { 
                    text: formatMessage('Only group admins can use the warn command.', '❌')
                });
                return;
            }
        } catch (adminError) {
            ...
            await sock.sendMessage(chatId, { 
                text: formatMessage('Please make sure the bot is an admin of this group.', '⚠️')
            });
            return;
        }
        ...
        if (!userToWarn) {
            await sock.sendMessage(chatId, { 
                text: formatMessage('Please mention the user or reply to their message to warn!', '❗')
            });
            return;
        }
        ...
        try {
            ...
            await sock.sendMessage(chatId, { 
                text: formatMessage('Failed to warn user!', '❌')
            });
        }
    } catch (error) {
        console.error('Error in warn command:', error);
