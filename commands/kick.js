...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: formatMessage('Only group admins can use the kick command.', '❌') }, { quoted: message });
            return;
        }
    }
    ...
    if (usersToKick.length === 0) {
        await sock.sendMessage(chatId, { 
            text: formatMessage('Please mention the user or reply to their message to kick!', '❗')
        }, { quoted: message });
        return;
    }
    ...
    if (isTryingToKickBot) {
        await sock.sendMessage(chatId, { 
            text: formatMessage("I can't kick myself 🤖", '🤖')
        }, { quoted: message });
        return;
    }
    ...
    try {
        ...
        await sock.sendMessage(chatId, { 
            text: formatMessage('Failed to kick user(s)!', '❌')
        });
    }
}
