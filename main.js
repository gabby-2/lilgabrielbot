...
global.channelLink = "https://whatsapp.com/channel/0029VbBM2CX47XeCRkActP3D";
global.ytch = "Lil Gabriel";
...
const { formatMessage } = require('./lib/messageConfig');
        ...
        // (Removed helper functions for private->group invocation to restore original behavior)
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '📢 *Join our Channel:*\nhttps://whatsapp.com/channel/0029VbBM2CX47XeCRkActP3D'
                }, { quoted: message });
                return;
        ...
        // List of admin commands (unprefixed)
        const adminCommands = ['mute', 'unmute', 'ban', 'unban', 'promote', 'demote', 'kick', 'tagall', 'tagnotadmin', 'hidetag', 'antilink', 'antitag', 'setgdesc', 'setgname', 'setgpp'];
        const adminCommandsPrefixed = adminCommands.map(c => prefix + c);
        const isAdminCommand = adminCommandsPrefixed.some(cmd => userMessage.startsWith(cmd));

        // List of owner commands (unprefixed)
        ...
        const ownerCommandsPrefixed = ownerCommands.map(c => prefix + c);
        const isOwnerCommand = ownerCommandsPrefixed.some(cmd => userMessage.startsWith(cmd));
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (!isBotAdmin) {
                        await sock.sendMessage(chatId, { text: formatMessage('Please make the bot an admin to use admin commands.', '⚠️'), ...channelInfo }, { quoted: message });
                return;
            }
        ...
        switch (true) {
            ...
            case userMessage.startsWith(prefix + 'setprefix'):
                {
                    ...
                    await setprefix(sock, chatId, message);
                }
                break;
            ...
            case userMessage.startsWith(prefix + 'tag'):
                const messageText = rawText.slice((prefix + 'tag').length).trim();  // use rawText here, not userMessage
                const replyMessage = message.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;
            ...
            case userMessage.startsWith(prefix + 'antilink'):
                {
                    if (!isGroup) {
                        await sock.sendMessage(chatId, { text: formatMessage('This command can only be used in groups.', '❗'), ...channelInfo }, { quoted: message });
                        return;
                    }
                    if (!isBotAdmin) {
                        await sock.sendMessage(chatId, { text: formatMessage('Please make the bot an admin first.', '⚠️'), ...channelInfo }, { quoted: message });
                        return;
                    }
